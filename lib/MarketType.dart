...
class shareTypePage extends StatefulWidget {
  @override
  _shareTypePageState createState() => _shareTypePageState();
}

class _shareTypePageState extends State<shareTypePage> {
    ...
    if (documentName.isNotEmpty) {
      if (documentNames.any((name) => name.toLowerCase() == documentName)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Share Type "$documentName" already exists.'),
        ));
      } else {
        FirebaseFirestore.instance.collection('shareType').doc(documentName).set({
  ...
  void _deleteDocument(String documentName) {
    FirebaseFirestore.instance.collection('shareType').doc(documentName).delete().then((_) {
